...
class SubjectType(models.TextChoices):
    """What a referral is about — WLT handoff decision D4, backlog S0.1.

    `CASE` is every referral the platform held before the group module: a young
    person, through the case that carries their pathway. `YOUTH` is a person
    referred outside a case, which is how a WLT member reaches a health or legal
    service without a youth-employment case being invented for her. The last
    three are collective subjects, and they are the reason the field exists: the
    core answers "what is happening to this person", and the group module asks
    ...
    @classmethod
    def person_level(cls):
        """Subjects that are one identifiable person.

        The distinction safeguarding turns on: a protection referral is
        permitted for these and for nothing else.
        """
        return {cls.CASE, cls.YOUTH}
...
class ReferralQuerySet(models.QuerySet):
    def youth_side(self):
        """Referrals that hang off a youth-employment case.

        Every queryset below this one, every §8 dashboard figure and every §4.13
        alert job means *these*. They are not narrowed individually: the
        viewsets and the dashboard scope through this so a group-subject
        referral cannot appear in a case worker's list by having been forgotten
        in one filter out of forty.
        """
        return self.filter(case__isnull=False)

    def with_subject_type(self, subject_type):
        return self.filter(subject_type=subject_type)
...
class Referral(BaseModel):
    ...
    # -- subject ----------------------------------------------------------
    #
    # A referral names exactly one subject. Until the WLT group module (handoff
    # decision D4, backlog S0.1) that subject was always a case, and for every
    # youth-side referral it still is — `case` is one of five slots rather than
    # a column that was replaced, which is why this migration needs no backfill,
    # no dual-write window and none of the eight stages the handoff's runbook
    # describes for a column swap. Existing rows already satisfy the constraint.
    #
    # Typed nullable columns plus an exactly-one check, not a GenericForeignKey.
    # Three reasons, all from D4 and all still true here: the reporting layer
    # joins these in SQL and a GFK needs a contenttypes lookup per row; a GFK
    # gives no referential integrity, so a deleted group would leave dangling
    # referrals; and four indexed columns read obviously in the SQL the team
    # already maintains. The cost is one column per subject type, which is
    # acceptable at five and would not be if subject types were open-ended.
    ...
    subject_youth = models.ForeignKey(
        "youth.Youth",
        null=True,
        blank=True,
        on_delete=models.PROTECT,
        related_name="subject_referrals",
        verbose_name=_("youth subject"),
        help_text=_("A person referred outside a youth-employment case — a WLT member, for instance."),
    )
    ...
    subject_federation = models.ForeignKey(
        ...
        on_delete=models.PROTECT,
        related_name="referrals",
        verbose_name=_("federation subject"),
    )
    # Stored and generated by the database, so it cannot disagree with the
    # columns it describes. Read-only in Django: assigning it would be assigning
    # to a derivation.
    subject_type = models.GeneratedField(
        expression=models.Case(
            models.When(case__isnull=False, then=models.Value(SubjectType.CASE)),
            models.When(subject_youth__isnull=False, then=models.Value(SubjectType.YOUTH)),
            models.When(subject_group__isnull=False, then=models.Value(SubjectType.GROUP)),
            models.When(subject_cla__isnull=False, then=models.Value(SubjectType.CLA)),
            models.When(subject_federation__isnull=False, then=models.Value(SubjectType.FEDERATION)),
    ...
    class Meta:
        constraints = [
            ...
            # Exactly one subject. This is the referential integrity a
            # GenericForeignKey cannot give: two subjects is ambiguous and zero
            # is a referral about nobody, and both were reachable through the
            # ORM before the column existed. Assertions A16 and A17.
            models.CheckConstraint(
                condition=(
                    models.Q(case__isnull=False)
                    & models.Q(subject_youth__isnull=True)
                    & models.Q(subject_group__isnull=True)
                    & models.Q(subject_cla__isnull=True)
                    & models.Q(subject_federation__isnull=True)
                )
                | (
                    models.Q(case__isnull=True)
                    & models.Q(subject_youth__isnull=False)
                    & models.Q(subject_group__isnull=True)
                    & models.Q(subject_cla__isnull=True)
                    & models.Q(subject_federation__isnull=True)
                )
                | (
                    models.Q(case__isnull=True)
                    & models.Q(subject_youth__isnull=True)
                    & models.Q(subject_group__isnull=False)
                    & models.Q(subject_cla__isnull=True)
                    & models.Q(subject_federation__isnull=True)
                )
                | (
                    ...
                    & models.Q(subject_youth__isnull=True)
                    & models.Q(subject_group__isnull=True)
                    & models.Q(subject_cla__isnull=False)
                    & models.Q(subject_federation__isnull=True)
                )
                | (
                    models.Q(case__isnull=True)
                    ...
                    & models.Q(subject_group__isnull=True)
                    & models.Q(subject_cla__isnull=True)
                    & models.Q(subject_federation__isnull=False)
                ),
        ...
        indexes = [
            ...
            # One partial index per non-case subject, so a group's linkage
            # history is a lookup rather than a scan of a table that is
            # overwhelmingly youth-side.
            models.Index(
            ...
            models.Index(
                fields=["subject_cla"], condition=models.Q(subject_cla__isnull=False), name="referral_subject_cla_idx"
            ),
            models.Index(
    ...
    def __str__(self):
        return f"{self.referral_category.label} referral for {self.subject_label}"

    # -- subject ----------------------------------------------------------

    SUBJECT_FIELDS = ("case", "subject_youth", "subject_group", "subject_cla", "subject_federation")

    @property
    def subject(self):
        """The one thing this referral is about."""
    ...
    def resolved_subject_type(self):
        """The subject type without a database round trip.

        `subject_type` is a stored generated column, so it is empty on an
        unsaved instance and stale on one whose subject was just reassigned.
        Validation runs before the save, so it reads this instead. The handoff
        records the same trap for its BEFORE trigger, where getting it wrong
        makes the safeguarding check silently pass everything.
        """
        if self.case_id:
            return SubjectType.CASE
        if self.subject_youth_id:
            return SubjectType.YOUTH
        if self.subject_group_id:
            return SubjectType.GROUP
        if self.subject_cla_id:
            return SubjectType.CLA
        if self.subject_federation_id:
            return SubjectType.FEDERATION
        return None

    @property
    def subject_label(self):
        """A name for the subject, whatever kind it is."""
        if self.case_id:
            return self.case.youth.full_name
        subject = self.subject
        return str(subject) if subject is not None else str(_("no subject"))
    ...
    def transition_to(self, new_status, actor=None, **kwargs):
        ...
        if new_status == ReferralStatus.COMPLETED and self.outcome_type_id and self.case_id:
        ...
        # A referral whose subject is a group or a person outside a case has no
        # case to stamp; the group module keeps its own activity record.
        if self.case_id:
            self.case.touch()
        return self
    ...
    def _join_or_open_parallel_group(self):
        ...
        if not self.case_id:
            # §6.3 caps the referrals running against one *case*. A group has no
            # case and no pathway, so there is no slot to occupy — and reading
            # the rule as "two active referrals per subject" would silently cap
            # a savings group at two linkages, which is a policy nobody set.
            return
    ...
    def clean(self):
        ...
        subject_type = self.resolved_subject_type
        subjects_named = sum(1 for field in self.SUBJECT_FIELDS if getattr(self, f"{field}_id", None))
        if subjects_named == 0:
            errors["case"] = _("A referral needs a subject: a case, a youth, an SHG, a CLA or a federation.")
        elif subjects_named > 1:
            errors["case"] = _("A referral names exactly one subject.")
        elif self.referral_category_id and not self.referral_category.permits(subject_type):
            # The safeguarding check. Phrased as what the category permits
            # rather than as a generic rejection, because the case it exists for
            # is a protection referral being raised against a group, and the
            # person doing it needs to understand why not.
            errors["referral_category"] = _("A %(category)s referral cannot be raised against a %(subject)s.") % {
                "category": self.referral_category.label,
                "subject": SubjectType(subject_type).label,
            }
