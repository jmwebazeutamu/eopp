...
const linkage = {
  ...
  provider: "p1",
  provider_name: "Afar Bank",
  subject_type: "GROUP",
  subject_name: "Hidase SHG",
  status: "SCREENED",
  status_display: "Screened",
...
function mount(
  role:
    | "WLT_FACILITATOR"
    | "WLT_WOREDA_OFFICER"
    | "SYSTEM_ADMIN" = "WLT_FACILITATOR",
) {
  get.mockImplementation((url?: string) => {
    if (!url) return Promise.resolve({ data: { count: 0, results: [] } });
    if (url.endsWith("summary/"))
      return Promise.resolve({ data: { total: 1, counters: [] } });
...
describe("LinkagesPage", () => {
  ...
  it("gives a facilitator the resolution step before submission and shows evidence", async () => {
    mount();
    fireEvent.click((await screen.findAllByText("Savings account"))[0]);
    expect(
      await screen.findByText("Record group resolution"),
    ).toBeInTheDocument();
    expect(screen.getByText("Submit for approval")).toBeInTheDocument();
    fireEvent.click(screen.getByText("View Evidence & immutable history"));
  ...
  it("does not expose facilitator actions to an approver", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText("View Evidence & immutable history"),
      ).toBeInTheDocument(),
    );
